...
static const struct facility_map {
  ...
  size_t match;
    ...
    LOG_MAIL, "mail", 1}, {LOG_DAEMON, "daemon", 1}, {LOG_AUTH, "auth", 1}, {
    LOG_SYSLOG, "syslog", 1}, {LOG_LPR, "lpr", 2}, {LOG_NEWS, "news", 1}, {
    LOG_UUCP, "uucp", 2}, {LOG_CRON, "cron", 1},
#ifdef LOG_FTP
    {LOG_FTP, "ftp", 1},
#endif
    {LOG_LOCAL0, "local0", 6}, {LOG_LOCAL1, "local1", 6}, {LOG_LOCAL2, "local2",
        6}, {LOG_LOCAL3, "local3", 6}, {LOG_LOCAL4, "local4", 6}, {LOG_LOCAL5,
        "local5", 6}, {LOG_LOCAL6, "local6", 6}, {LOG_LOCAL7, "local7", 6}, {0,
        NULL, 0}, };
...
static int config_write_host(struct vty *vty) {
  ...
  if (zlog_default->facility != LOG_DAEMON)
    vty_out(vty, "log facility %s%s", facility_name(zlog_default->facility),
        VTY_NEWLINE);
...
/* Configration from terminal */DEFUN (config_terminal,
    config_terminal_cmd,
    "configure terminal",
...
/* Down vty node level. */DEFUN (config_exit,
    config_exit_cmd,
    "exit",
...
ALIAS(config_exit, config_quit_cmd, "quit",
    "Exit current mode and down to previous mode\n")

/* End of configuration. */DEFUN (config_end,
    config_end_cmd,
    "end",
...
/* Show version. */DEFUN (show_version,
    show_version_cmd,
    "show version",
    SHOW_STR
...
/* Help display function for all node. */DEFUN (config_help,
    config_help_cmd,
    "help",
...
/* Help display function for all node. */DEFUN (config_list,
    config_list_cmd,
    "list",
    "Print command list\n") {
  unsigned int i;
...
/* Write current configuration into file. */DEFUN (config_write_file,
  ...
  if (chmod(config_file, CONFIGFILE_MASK) != 0) {
    vty_out(vty, "Can't chmod configuration file %s: %s (%d).%s", config_file,
        safe_strerror(errno), errno, VTY_NEWLINE);
    goto finished;
  }
...
ALIAS(config_write_file, config_write_cmd, "write",
    "Write running configuration to memory, network, or terminal\n")
...
ALIAS(config_write_file, copy_runningconfig_startupconfig_cmd,
    "copy running-config startup-config", "Copy configuration\n"
        "Copy running config to... \n"
        "Copy running config to startup config (same as write file)\n")

/* Write current configuration into the terminal. */DEFUN (config_write_terminal,
...
ALIAS(config_write_terminal, show_running_config_cmd, "show running-config",
    SHOW_STR "running configuration\n")

/* Write startup configuration into the terminal. */DEFUN (show_startup_config,
...
/* Hostname configuration */DEFUN (config_hostname,
    hostname_cmd,
    "hostname WORD",
...
/* VTY interface password set. */DEFUN (config_password, password_cmd,
    "password (8|) WORD",
...
/* VTY enable password set. */DEFUN (config_enable_password, enable_password_cmd,
...
ALIAS(no_config_log_file, no_config_log_file_level_cmd,
...
ALIAS(no_config_log_syslog, no_config_log_syslog_facility_cmd,
