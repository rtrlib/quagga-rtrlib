...
static int bgp_export_modifier(struct peer *rsclient, struct peer *peer,
...
static int bgp_import_modifier(struct peer *rsclient, struct peer *peer,
    struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
  struct bgp_filter *filter;
  struct bgp_info info;
  route_map_result_t ret;
...
static int bgp_announce_check(struct bgp_info *ri, struct peer *peer,
    struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
  int ret;
  ...
  if (bgp_output_filter(peer, p, riattr, afi, safi) == FILTER_DENY) {
    if (BGP_DEBUG (filter, FILTER))
      zlog(peer->log, LOG_DEBUG, "%s [Update:SEND] %s/%d is filtered",
          peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
          p->prefixlen);
    return 0;
  }
  ...
  if (peer->sort
      == BGP_PEER_EBGP && attr->flag & ATTR_FLAG_BIT (BGP_ATTR_MULTI_EXIT_DISC)) {
    if (ri->peer != bgp->peer_self&& ! transparent
    && ! CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_MED_UNCHANGED))attr ->
      flag &= ~(ATTR_FLAG_BIT (BGP_ATTR_MULTI_EXIT_DISC));
  }
...
static int bgp_announce_check_rsclient(struct bgp_info *ri,
    struct peer *rsclient, struct prefix *p, struct attr *attr, afi_t afi,
    safi_t safi) {
  int ret;
  char buf[SU_ADDRSTRLEN];
  struct bgp_filter *filter;
  ...
  if (bgp_output_filter(rsclient, p, riattr, afi, safi) == FILTER_DENY) {
    if (BGP_DEBUG (filter, FILTER))
      zlog(rsclient->log, LOG_DEBUG, "%s [Update:SEND] %s/%d is filtered",
          rsclient->host,
...
static void bgp_best_selection(struct bgp *bgp, struct bgp_node *rn,
    struct bgp_maxpaths_cfg *mpath_cfg, struct bgp_info_pair *result) {
...
static int bgp_process_announce_selected(struct peer *peer,
    struct bgp_info *selected, struct bgp_node *rn, afi_t afi, safi_t safi) {
  struct prefix *p;
  struct attr attr;
...
static void bgp_process_queue_init(void) {
  ...
  bm->process_rsclient_queue = work_queue_new(bm->master,
...
void bgp_process(struct bgp *bgp, struct bgp_node *rn, afi_t afi, safi_t safi) {
  ...
  (bm->process_rsclient_queue == NULL) )bgp_process_queue_init ();
...
static void bgp_update_rsclient(struct peer *rsclient, afi_t afi, safi_t safi,
  ...
  /* Apply export policy. */
  if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)
      && bgp_export_modifier(rsclient, peer, p, &new_attr, afi, safi)
          == RMAP_DENY) {
  ...
  if ((afi == AFI_IP) && ((safi == SAFI_UNICAST) || safi == SAFI_MULTICAST)) {
    ...
    if (new_attr.nexthop.s_addr
        == 0|| IPV4_CLASS_DE (ntohl (new_attr.nexthop.s_addr))) {
  ...
  if (ri) {
    ...
    if (!CHECK_FLAG(ri->flags, BGP_INFO_REMOVED)
      ...
      if (BGP_DEBUG (update, UPDATE_IN))
        zlog(peer->log, LOG_DEBUG,
            "%s rcvd %s/%d for RS-client %s...duplicate ignored", peer->host,
            inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
    ...
    /* Received Logging. */
    if (BGP_DEBUG (update, UPDATE_IN))
      zlog(peer->log, LOG_DEBUG, "%s rcvd %s/%d for RS-client %s", peer->host,
          inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen,
          rsclient->host);
...
static void bgp_withdraw_rsclient(struct peer *rsclient, afi_t afi, safi_t safi,
  ...
  if (ri && !CHECK_FLAG (ri->flags, BGP_INFO_HISTORY))
    ...
    zlog(peer->log, LOG_DEBUG, "%s Can't find the route %s/%d", peer->host,
        inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
...
static int bgp_update_main(struct peer *peer, struct prefix *p,
  ...
  if (afi == AFI_IP && safi == SAFI_UNICAST) {
    ...
    && ! CHECK_FLAG (peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)) {reason="non-connected next-hop;";
    goto filtered;
  }
  ...
  if (ri) {
    ...
    if (!CHECK_FLAG (ri->flags, BGP_INFO_REMOVED)
      ...
      if (CHECK_FLAG (bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING)
          && peer->sort
              == BGP_PEER_EBGP&& CHECK_FLAG (ri->flags, BGP_INFO_HISTORY)) {
        if (BGP_DEBUG (update, UPDATE_IN))
          zlog(peer->log, LOG_DEBUG, "%s rcvd %s/%d", peer->host,
              inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
              p->prefixlen);
    ...
    if (CHECK_FLAG(ri->flags, BGP_INFO_REMOVED)) {
      if (BGP_DEBUG (update, UPDATE_IN))
        zlog(peer->log, LOG_DEBUG,
            "%s rcvd %s/%d, flapped quicker than processing", peer->host,
            inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
            p->prefixlen);
      bgp_info_restore(rn, ri);
    }
    ...
    if ((afi == AFI_IP || afi == AFI_IP6) && safi == SAFI_UNICAST
        && (peer->sort == BGP_PEER_IBGP || peer->sort == BGP_PEER_CONFED
  ...
  /* This BGP update is filtered.  Log the reason then update BGP
   entry.  */
  filtered: if (BGP_DEBUG (update, UPDATE_IN))
    zlog(peer->log, LOG_DEBUG,
        "%s rcvd UPDATE about %s/%d -- DENIED due to: %s", peer->host,
        inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen,
        reason);
...
int bgp_update(struct peer *peer, struct prefix *p, struct attr *attr,
  ...
  for (ALL_LIST_ELEMENTS(bgp->rsclient, node, nnode, rsclient)) {
    if (CHECK_FLAG (rsclient->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
      bgp_update_rsclient(rsclient, afi, safi, attr, peer, p, type, sub_type,
          prd, tag);
  }
...
static void bgp_announce_table(struct peer *peer, afi_t afi, safi_t safi,
...
static void bgp_soft_reconfig_table_rsclient(struct peer *rsclient, afi_t afi,
  ...
  for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
    for (ain = rn->adj_in; ain; ain = ain->next) {
      ...
      bgp_update_rsclient(rsclient, afi, safi, ain->attr, ain->peer, &rn->p,
          ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, prd, tag);
    }
}
...
void bgp_soft_reconfig_rsclient(struct peer *rsclient, afi_t afi, safi_t safi) {
  ...
  if (safi != SAFI_MPLS_VPN)
    ...
    for (rn = bgp_table_top(rsclient->bgp->rib[afi][safi]); rn; rn =
        bgp_route_next(rn))
      if ((table = rn->info) != NULL ) {
  ...
  for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
    for (ain = rn->adj_in; ain; ain = ain->next) {
      if (ain->peer == peer) {
        ...
        ret = bgp_update(peer, &rn->p, ain->attr, afi, safi, ZEBRA_ROUTE_BGP,
  ...
  for (ri = rn->info; ri; ri = ri->next)
    if (ri->peer == peer || cnq->purpose == BGP_CLEAR_ROUTE_MY_RSCLIENT) {
      ...
      if (CHECK_FLAG (peer->sflags, PEER_STATUS_NSF_WAIT)
          && peer->nsf[afi][safi] && !CHECK_FLAG (ri->flags, BGP_INFO_STALE)
  ...
  switch (purpose) {
    case BGP_CLEAR_ROUTE_NORMAL:
      if (safi != SAFI_MPLS_VPN)
        ...
        for (rn = bgp_table_top(peer->bgp->rib[afi][safi]); rn; rn =
            bgp_route_next(rn))
          if ((table = rn->info) != NULL )
...
void bgp_cleanup_routes(void) {
  ...
  for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
    ...
    for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
      for (ri = rn->info; ri; ri = ri->next)
        if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED)
            && ri->type == ZEBRA_ROUTE_BGP && ri->sub_type == BGP_ROUTE_NORMAL)
  ...
  for (; pnt < lim; pnt += psize) {
    ...
    if (packet->afi == AFI_IP && packet->safi == SAFI_UNICAST) {
      if (IN_CLASSD (ntohl (p.u.prefix4.s_addr))) {
         ...
         */
        zlog(peer->log, LOG_ERR, "IPv4 unicast NLRI is multicast address %s",
...
int bgp_nlri_sanity_check(struct peer *peer, int afi, u_char *pnt,
  ...
  while (pnt < end) {
    ...
    if ((afi == AFI_IP && prefixlen > 32)
      ...
      plog_err(peer->log,
          "%s [Error] Update packet error (wrong prefix length %d)", peer->host,
          prefixlen);
    ...
    if (pnt + psize > end) {
      plog_err(peer->log, "%s [Error] Update packet error"
          " (prefix data overflow prefix size is %d)", peer->host, psize);
      bgp_notify_send(peer, BGP_NOTIFY_UPDATE_ERR,
          BGP_NOTIFY_UPDATE_INVAL_NETWORK);
      return -1;
    }
  ...
  if (pnt != end) {
    plog_err(peer->log, "%s [Error] Update packet error"
        " (prefix length mismatch with total length)", peer->host);
    bgp_notify_send(peer, BGP_NOTIFY_UPDATE_ERR,
        BGP_NOTIFY_UPDATE_INVAL_NETWORK);
    return -1;
  }
...
static void bgp_static_withdraw_rsclient(struct bgp *bgp, struct peer *rsclient,
...
static void bgp_static_update_rsclient(struct peer *rsclient, struct prefix *p,
  ...
  if (ri) {
    ...
    !CHECK_FLAG(ri->flags, BGP_INFO_REMOVED)) {bgp_unlock_node
      (rn);
      bgp_attr_unintern(&attr_new);
      aspath_unintern(&attr.aspath);
      bgp_attr_extra_free(&attr);
...
static void bgp_static_update_vpnv4(struct bgp *bgp, struct prefix *p,
    afi_t afi, safi_t safi, struct prefix_rd *prd, u_char *tag) {
  struct bgp_node *rn;
  struct bgp_info *new;
...
void bgp_check_local_routes_rsclient(struct peer *rsclient, afi_t afi,
    safi_t safi) {
...
static void bgp_static_withdraw_vpnv4(struct bgp *bgp, struct prefix *p,
    afi_t afi, safi_t safi, struct prefix_rd *prd, u_char *tag) {
  struct bgp_node *rn;
  struct bgp_info *ri;
...
static int bgp_static_set(struct vty *vty, struct bgp *bgp, const char *ip_str,
...
static int bgp_static_unset(struct vty *vty, struct bgp *bgp,
        ...
        if (rn->info != NULL ) {
          if (safi == SAFI_MPLS_VPN) {
            ...
            for (rm = bgp_table_top(table); rm; rm = bgp_route_next(rm)) {
              bgp_static = rn->info;
              bgp_static_withdraw_vpnv4(bgp, &rm->p, AFI_IP, SAFI_MPLS_VPN,
                  (struct prefix_rd *) &rn->p, bgp_static->tag);
              bgp_static_free(bgp_static);
              rn->info = NULL;
              bgp_unlock_node(rn);
            }
...
int bgp_static_set_vpnv4(struct vty *vty, const char *ip_str,
  ...
  prn = bgp_node_get(bgp->route[AFI_IP][SAFI_MPLS_VPN], (struct prefix *) &prd);
  if (prn->info == NULL )
...
DEFUN (bgp_network,
  ...
  return bgp_static_set(vty, vty->index, argv[0], AFI_IP, bgp_node_safi(vty),
      NULL, 0);
}
...
DEFUN (bgp_network_mask,
  ...
  return bgp_static_set(vty, vty->index, prefix_str, AFI_IP, bgp_node_safi(vty),
      NULL, 0);
}
...
DEFUN (bgp_network_mask_backdoor,
  ...
  return bgp_static_set(vty, vty->index, prefix_str, AFI_IP, SAFI_UNICAST, NULL,
      1);
}
...
ALIAS(no_bgp_network, no_bgp_network_route_map_cmd,
    "no network A.B.C.D/M route-map WORD",
    NO_STR "Specify a network to announce via BGP\n" "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n" "Route-map to modify the attributes\n" "Name of the route map\n")

ALIAS(no_bgp_network, no_bgp_network_backdoor_cmd,
...
ALIAS(no_bgp_network_mask, no_bgp_network_mask_route_map_cmd,
    "no network A.B.C.D mask A.B.C.D route-map WORD",
    NO_STR "Specify a network to announce via BGP\n" "Network number\n" "Network mask\n" "Network mask\n" "Route-map to modify the attributes\n" "Name of the route map\n")

ALIAS(no_bgp_network_mask, no_bgp_network_mask_backdoor_cmd,
...
ALIAS(no_bgp_network_mask_natural, no_bgp_network_mask_natural_route_map_cmd,
    "no network A.B.C.D route-map WORD",
    NO_STR "Specify a network to announce via BGP\n" "Network number\n" "Route-map to modify the attributes\n" "Name of the route map\n")

ALIAS(no_bgp_network_mask_natural, no_bgp_network_mask_natural_backdoor_cmd,
...
ALIAS(no_ipv6_bgp_network, no_ipv6_bgp_network_route_map_cmd,
...
ALIAS(no_ipv6_bgp_network, old_no_ipv6_bgp_network_cmd,
...
ALIAS_DEPRECATED(bgp_network, bgp_network_ttl_cmd,
    "network A.B.C.D/M pathlimit <0-255>",
    "Specify a network to announce via BGP\n"
        "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
        "AS-Path hopcount limit attribute\n"
        "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(bgp_network_backdoor, bgp_network_backdoor_ttl_cmd,
    ...
    "Specify a network to announce via BGP\n"
        ...
        "AS-Path hopcount limit attribute\n"
        "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(bgp_network_mask, bgp_network_mask_ttl_cmd,
    ...
    "Specify a network to announce via BGP\n"
        ...
        "AS-Path hopcount limit attribute\n"
        "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(bgp_network_mask_backdoor, bgp_network_mask_backdoor_ttl_cmd,
    ...
    "Specify a network to announce via BGP\n"
        ...
        "AS-Path hopcount limit attribute\n"
        "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(bgp_network_mask_natural, bgp_network_mask_natural_ttl_cmd,
...
ALIAS_DEPRECATED(bgp_network_mask_natural_backdoor,
    ...
    "Specify a network to announce via BGP\n"
        ...
        "AS-Path hopcount limit attribute\n"
        "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_bgp_network, no_bgp_network_ttl_cmd,
    "no network A.B.C.D/M pathlimit <0-255>",
    NO_STR "Specify a network to announce via BGP\n" "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n" "AS-Path hopcount limit attribute\n" "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_bgp_network, no_bgp_network_backdoor_ttl_cmd,
    "no network A.B.C.D/M backdoor pathlimit <0-255>",
    NO_STR "Specify a network to announce via BGP\n" "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n" "Specify a BGP backdoor route\n" "AS-Path hopcount limit attribute\n" "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_bgp_network, no_bgp_network_mask_ttl_cmd,
    "no network A.B.C.D mask A.B.C.D pathlimit <0-255>",
    NO_STR "Specify a network to announce via BGP\n" "Network number\n" "Network mask\n" "Network mask\n" "AS-Path hopcount limit attribute\n" "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_bgp_network_mask, no_bgp_network_mask_backdoor_ttl_cmd,
    "no network A.B.C.D mask A.B.C.D  backdoor pathlimit <0-255>",
    NO_STR "Specify a network to announce via BGP\n" "Network number\n" "Network mask\n" "Network mask\n" "Specify a BGP backdoor route\n" "AS-Path hopcount limit attribute\n" "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_bgp_network_mask_natural,
    no_bgp_network_mask_natural_ttl_cmd, "no network A.B.C.D pathlimit <0-255>",
    NO_STR "Specify a network to announce via BGP\n" "Network number\n" "AS-Path hopcount limit attribute\n" "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_bgp_network_mask_natural,
    ...
    NO_STR "Specify a network to announce via BGP\n" "Network number\n" "Specify a BGP backdoor route\n" "AS-Path hopcount limit attribute\n" "AS-Pathlimit TTL, in number of AS-Path hops\n")
#ifdef HAVE_IPV6
ALIAS_DEPRECATED(ipv6_bgp_network, ipv6_bgp_network_ttl_cmd,
    "network X:X::X:X/M pathlimit <0-255>",
    "Specify a network to announce via BGP\n"
        "IPv6 prefix <network>/<length>\n"
        "AS-Path hopcount limit attribute\n"
        "AS-Pathlimit TTL, in number of AS-Path hops\n")
ALIAS_DEPRECATED(no_ipv6_bgp_network, no_ipv6_bgp_network_ttl_cmd,
...
static void bgp_aggregate_route(struct bgp *bgp, struct prefix *p,
    struct bgp_info *rinew, afi_t afi, safi_t safi, struct bgp_info *del,
...
DEFUN (aggregate_address_mask_as_set,
  ...
  return bgp_aggregate_set(vty, prefix_str, AFI_IP, bgp_node_safi(vty), 0,
      AGGREGATE_AS_SET);
}
...
ALIAS(aggregate_address_as_set_summary, aggregate_address_summary_as_set_cmd,
...
ALIAS(no_aggregate_address, no_aggregate_address_summary_only_cmd,
    "no aggregate-address A.B.C.D/M summary-only",
    NO_STR "Configure BGP aggregate entries\n" "Aggregate prefix\n" "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address, no_aggregate_address_as_set_cmd,
    "no aggregate-address A.B.C.D/M as-set",
    NO_STR "Configure BGP aggregate entries\n" "Aggregate prefix\n" "Generate AS set path information\n")

ALIAS(no_aggregate_address, no_aggregate_address_as_set_summary_cmd,
    "no aggregate-address A.B.C.D/M as-set summary-only",
    NO_STR "Configure BGP aggregate entries\n" "Aggregate prefix\n" "Generate AS set path information\n" "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address, no_aggregate_address_summary_as_set_cmd,
...
ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_summary_only_cmd,
    "no aggregate-address A.B.C.D A.B.C.D summary-only",
    NO_STR "Configure BGP aggregate entries\n" "Aggregate address\n" "Aggregate mask\n" "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_as_set_cmd,
    "no aggregate-address A.B.C.D A.B.C.D as-set",
    NO_STR "Configure BGP aggregate entries\n" "Aggregate address\n" "Aggregate mask\n" "Generate AS set path information\n")

ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_as_set_summary_cmd,
    "no aggregate-address A.B.C.D A.B.C.D as-set summary-only",
    NO_STR "Configure BGP aggregate entries\n" "Aggregate address\n" "Aggregate mask\n" "Generate AS set path information\n" "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_summary_as_set_cmd,
...
ALIAS(ipv6_aggregate_address, old_ipv6_aggregate_address_cmd,
...
ALIAS(no_ipv6_aggregate_address, old_no_ipv6_aggregate_address_cmd,
  ...
  for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
    ...
    if (bgp->redist[afi][type]) {
      ...
      if (bi) {
        ...
        !CHECK_FLAG(bi->flags, BGP_INFO_REMOVED)) {bgp_attr_unintern
          (&new_attr);
          aspath_unintern(&attr.aspath);
          bgp_attr_extra_free(&attr);
          bgp_unlock_node(bn);
...
static void route_vty_out_detail(struct vty *vty, struct bgp *bgp,
    struct prefix *p, struct bgp_info *binfo, afi_t afi, safi_t safi) {
  ...
  if (attr) {
    ...
    if (CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_AGGREGATOR)))
      vty_out(vty, ", (aggregated by %u %s)", attr->extra->aggregator_as,
    ...
    if (p->family == AF_INET) {
      ...
      assert(attr->extra);
      vty_out(vty, "    %s",
          inet_ntop(AF_INET6, &attr->extra->mp_nexthop_global, buf,
              INET6_ADDRSTRLEN));
    }
#endif /* HAVE_IPV6 */
    ...
    if (attr->extra && attr->extra->mp_nexthop_len == 32) {
      vty_out(vty, "    (%s)%s",
          inet_ntop(AF_INET6, &attr->extra->mp_nexthop_local, buf,
...
static int bgp_show_table(struct vty *vty, struct bgp_table *table,
    struct in_addr *router_id, enum bgp_show_type type, void *output_arg) {
  struct bgp_info *ri;
  struct bgp_node *rn;
  int header = 1;
  int display;
  ...
  if (output_count == 0) {
    ...
    vty_out(vty, "%sTotal number of prefixes %ld%s", VTY_NEWLINE, output_count,
        VTY_NEWLINE);
...
static void route_vty_out_detail_header(struct vty *vty, struct bgp *bgp,
    struct bgp_node *rn, struct prefix_rd *prd, afi_t afi, safi_t safi) {
  struct bgp_info *ri;
  struct prefix *p;
  struct peer *peer;
  struct listnode *node, *nnode;
  ...
  vty_out(vty, "BGP routing table entry for %s%s%s/%d%s",
      ...
      safi == SAFI_MPLS_VPN ? ":" : "",
      inet_ntop(p->family, &p->u.prefix, buf2, INET6_ADDRSTRLEN), p->prefixlen,
      VTY_NEWLINE);
...
static int bgp_show_route_in_table(struct vty *vty, struct bgp *bgp,
    struct bgp_table *rib, const char *ip_str, afi_t afi, safi_t safi,
          ...
          for (ri = rm->info; ri; ri = ri->next) {
            if (header) {
              route_vty_out_detail_header(vty, bgp, rm,
                  (struct prefix_rd *) &rn->p, AFI_IP, SAFI_MPLS_VPN);
...
static int bgp_show_route(struct vty *vty, const char *view_name,
    const char *ip_str, afi_t afi, safi_t safi, struct prefix_rd *prd,
    int prefix_check) {
  struct bgp *bgp;
  ...
  return bgp_show_route_in_table(vty, bgp, bgp->rib[afi][safi], ip_str, afi,
      safi, prd, prefix_check);
}

/* BGP route print out function. */DEFUN (show_ip_bgp,
    show_ip_bgp_cmd,
    "show ip bgp",
    SHOW_STR
    IP_STR
...
ALIAS(show_ip_bgp_ipv4, show_bgp_ipv4_safi_cmd,
...
ALIAS(show_ip_bgp_ipv4_route, show_bgp_ipv4_safi_route_cmd,
...
ALIAS(show_ip_bgp_ipv4_prefix, show_bgp_ipv4_safi_prefix_cmd,
...
ALIAS(show_bgp, show_bgp_ipv6_cmd, "show bgp ipv6",
    SHOW_STR BGP_STR "Address family\n")
...
/* old command */DEFUN (show_ipv6_bgp,
    show_ipv6_bgp_cmd,
    "show ipv6 bgp",
    SHOW_STR
    IP_STR
...
ALIAS(show_bgp_route, show_bgp_ipv6_route_cmd, "show bgp ipv6 X:X::X:X",
    SHOW_STR BGP_STR "Address family\n" "Network in the BGP routing table to display\n")
...
/* old command */DEFUN (show_ipv6_bgp_route,
...
ALIAS(show_bgp_prefix, show_bgp_ipv6_prefix_cmd, "show bgp ipv6 X:X::X:X/M",
    SHOW_STR BGP_STR "Address family\n" "IPv6 prefix <network>/<length>\n")
...
/* old command */DEFUN (show_ipv6_bgp_prefix,
...
ALIAS(show_bgp_view, show_bgp_view_ipv6_cmd, "show bgp view WORD ipv6",
...
ALIAS(show_bgp_view_route, show_bgp_view_ipv6_route_cmd,
...
ALIAS(show_bgp_view_prefix, show_bgp_view_ipv6_prefix_cmd,
    "show bgp view WORD ipv6 X:X::X:X/M",
    SHOW_STR BGP_STR "BGP view\n" "View name\n" "Address family\n" "IPv6 prefix <network>/<length>\n")

/* old command */DEFUN (show_ipv6_mbgp,
    show_ipv6_mbgp_cmd,
    "show ipv6 mbgp",
    SHOW_STR
    IP_STR
...
/* old command */DEFUN (show_ipv6_mbgp_route,
...
/* old command */DEFUN (show_ipv6_mbgp_prefix,
...
static int bgp_show_regexp(struct vty *vty, int argc, const char **argv,
...
ALIAS(show_bgp_regexp, show_bgp_ipv6_regexp_cmd, "show bgp ipv6 regexp .LINE",
    SHOW_STR BGP_STR "Address family\n" "Display routes matching the AS path regular expression\n" "A regular-expression to match the BGP AS paths\n")

/* old command */DEFUN (show_ipv6_bgp_regexp,
...
/* old command */DEFUN (show_ipv6_mbgp_regexp,
...
static int bgp_show_prefix_list(struct vty *vty, const char *prefix_list_str,
    afi_t afi, safi_t safi, enum bgp_show_type type) {
  struct prefix_list *plist;

  plist = prefix_list_lookup(afi, prefix_list_str);
  if (plist == NULL ) {
    vty_out(vty, "%% %s is not a valid prefix-list name%s", prefix_list_str,
        VTY_NEWLINE);
    return CMD_WARNING;
  }
...
ALIAS(show_bgp_prefix_list, show_bgp_ipv6_prefix_list_cmd,
    "show bgp ipv6 prefix-list WORD",
    SHOW_STR BGP_STR "Address family\n" "Display routes conforming to the prefix-list\n" "IPv6 prefix-list name\n")

/* old command */DEFUN (show_ipv6_bgp_prefix_list,
...
/* old command */DEFUN (show_ipv6_mbgp_prefix_list,
...
static int bgp_show_filter_list(struct vty *vty, const char *filter, afi_t afi,
    safi_t safi, enum bgp_show_type type) {
  struct as_list *as_list;

  as_list = as_list_lookup(filter);
  if (as_list == NULL ) {
    vty_out(vty, "%% %s is not a valid AS-path access-list name%s", filter,
        VTY_NEWLINE);
    return CMD_WARNING;
  }
...
ALIAS(show_bgp_filter_list, show_bgp_ipv6_filter_list_cmd,
    "show bgp ipv6 filter-list WORD",
    SHOW_STR BGP_STR "Address family\n" "Display routes conforming to the filter-list\n" "Regular expression access list name\n")

/* old command */DEFUN (show_ipv6_bgp_filter_list,
...
/* old command */DEFUN (show_ipv6_mbgp_filter_list,
...
static int bgp_show_route_map(struct vty *vty, const char *rmap_str, afi_t afi,
    safi_t safi, enum bgp_show_type type) {
...
ALIAS(show_bgp_route_map, show_bgp_ipv6_route_map_cmd,
...
DEFUN (show_ip_bgp_cidr_only,
  ...
  return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_cidr_only,
      NULL );
}
...
DEFUN (show_ip_bgp_flap_cidr_only,
    ...
    "Display flap statistics of routes\n"
    "Display only routes with non-natural netmasks\n") {
  return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_flap_cidr_only,
      NULL );
}
...
DEFUN (show_ip_bgp_ipv4_cidr_only,
    ...
    "Display only routes with non-natural netmasks\n") {
  if (strncmp(argv[0], "m", 1) == 0)
    return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_cidr_only,
        NULL );
...
DEFUN (show_ip_bgp_community_all,
  ...
  return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_community_all,
      NULL );
}
...
DEFUN (show_bgp_community_all,
    ...
    BGP_STR
    "Display routes matching the communities\n") {
  return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_community_all,
      NULL );
}

ALIAS(show_bgp_community_all, show_bgp_ipv6_community_all_cmd,
    "show bgp ipv6 community",
    SHOW_STR BGP_STR "Address family\n" "Display routes matching the communities\n")

/* old command */DEFUN (show_ipv6_bgp_community_all,
...
/* old command */DEFUN (show_ipv6_mbgp_community_all,
...
static int bgp_show_community(struct vty *vty, const char *view_name, int argc,
...
ALIAS(show_ip_bgp_community, show_ip_bgp_community2_cmd,
...
ALIAS(show_ip_bgp_community_exact, show_ip_bgp_community3_exact_cmd,
...
ALIAS(show_bgp_community, show_bgp_community2_cmd,
...
ALIAS(show_bgp_community, show_bgp_ipv6_community2_cmd,
    "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
    SHOW_STR BGP_STR "Address family\n" "Display routes matching the communities\n" "community number\n" "Do not send outside local AS (well-known community)\n" "Do not advertise to any peer (well-known community)\n" "Do not export to next AS (well-known community)\n" "community number\n" "Do not send outside local AS (well-known community)\n" "Do not advertise to any peer (well-known community)\n" "Do not export to next AS (well-known community)\n")

ALIAS(show_bgp_community, show_bgp_community3_cmd,
...
ALIAS(show_bgp_community, show_bgp_community4_cmd,
...
/* old command */DEFUN (show_ipv6_bgp_community,
...
/* old command */
ALIAS(show_ipv6_bgp_community, show_ipv6_bgp_community2_cmd,
...
/* old command */
ALIAS(show_ipv6_bgp_community, show_ipv6_bgp_community3_cmd,
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_community3_exact_cmd,
...
/* old command */DEFUN (show_ipv6_bgp_community_exact,
...
/* old command */
ALIAS(show_ipv6_bgp_community_exact, show_ipv6_bgp_community2_exact_cmd,
    "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
    SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n" "community number\n" "Do not send outside local AS (well-known community)\n" "Do not advertise to any peer (well-known community)\n" "Do not export to next AS (well-known community)\n" "community number\n" "Do not send outside local AS (well-known community)\n" "Do not advertise to any peer (well-known community)\n" "Do not export to next AS (well-known community)\n" "Exact match of the communities")

/* old command */
ALIAS(show_ipv6_bgp_community_exact, show_ipv6_bgp_community3_exact_cmd,
...
/* old command */DEFUN (show_ipv6_mbgp_community,
...
/* old command */
ALIAS(show_ipv6_mbgp_community, show_ipv6_mbgp_community2_cmd,
...
/* old command */
ALIAS(show_ipv6_mbgp_community, show_ipv6_mbgp_community3_cmd,
...
/* old command */DEFUN (show_ipv6_mbgp_community_exact,
...
/* old command */
ALIAS(show_ipv6_mbgp_community_exact, show_ipv6_mbgp_community2_exact_cmd,
    "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
    SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n" "community number\n" "Do not send outside local AS (well-known community)\n" "Do not advertise to any peer (well-known community)\n" "Do not export to next AS (well-known community)\n" "community number\n" "Do not send outside local AS (well-known community)\n" "Do not advertise to any peer (well-known community)\n" "Do not export to next AS (well-known community)\n" "Exact match of the communities")

/* old command */
ALIAS(show_ipv6_mbgp_community_exact, show_ipv6_mbgp_community3_exact_cmd,
...
static int bgp_show_community_list(struct vty *vty, const char *com, int exact,
    afi_t afi, safi_t safi) {
  ...
  return bgp_show(vty, NULL, afi, safi,
      (exact ? bgp_show_type_community_list_exact : bgp_show_type_community_list),
      list);
}
...
ALIAS(show_bgp_community_list, show_bgp_ipv6_community_list_cmd,
    "show bgp ipv6 community-list (<1-500>|WORD)",
    SHOW_STR BGP_STR "Address family\n" "Display routes matching the community-list\n" "community-list number\n" "community-list name\n")

/* old command */DEFUN (show_ipv6_bgp_community_list,
...
/* old command */DEFUN (show_ipv6_mbgp_community_list,
...
ALIAS(show_bgp_community_list_exact, show_bgp_ipv6_community_list_exact_cmd,
    "show bgp ipv6 community-list (<1-500>|WORD) exact-match",
    SHOW_STR BGP_STR "Address family\n" "Display routes matching the community-list\n" "community-list number\n" "community-list name\n" "Exact match of the communities\n")

/* old command */DEFUN (show_ipv6_bgp_community_list_exact,
...
/* old command */DEFUN (show_ipv6_mbgp_community_list_exact,
...
static int bgp_show_prefix_longer(struct vty *vty, const char *prefix,
    afi_t afi, safi_t safi, enum bgp_show_type type) {
  int ret;
  struct prefix *p;
...
ALIAS(show_bgp_prefix_longer, show_bgp_ipv6_prefix_longer_cmd,
...
/* old command */DEFUN (show_ipv6_mbgp_prefix_longer,
...
static struct peer *
peer_lookup_in_view(struct vty *vty, const char *view_name, const char *ip_str) {
  int ret;
  struct bgp *bgp;
  struct peer *peer;
  union sockunion su;
...
static const char *table_stats_strs[] = {
    [BGP_STATS_PREFIXES] = "Total Prefixes", [BGP_STATS_TOTPLEN
        ] = "Average prefix length", [BGP_STATS_RIB] = "Total Advertisements",
    [BGP_STATS_UNAGGREGATEABLE] = "Unaggregateable prefixes",
    [BGP_STATS_MAX_AGGREGATEABLE] = "Maximum aggregateable prefixes",
    [BGP_STATS_AGGREGATES] = "BGP Aggregate advertisements", [BGP_STATS_SPACE
        ] = "Address space advertised", [BGP_STATS_ASPATH_COUNT
        ] = "Advertisements with paths", [BGP_STATS_ASPATH_MAXHOPS
        ] = "Longest AS-Path (hops)", [BGP_STATS_ASPATH_MAXSIZE
        ] = "Largest AS-Path (bytes)", [BGP_STATS_ASPATH_TOTHOPS
        ] = "Average AS-Path length (hops)", [BGP_STATS_ASPATH_TOTSIZE
        ] = "Average AS-Path size (bytes)", [BGP_STATS_ASN_HIGHEST
        ] = "Highest public ASN", [BGP_STATS_MAX] = NULL,
  };
...
static int bgp_table_stats_walker(struct thread *t) {
  ...
  for (rn = top; rn; rn = bgp_route_next(rn)) {
    ...
    for (ri = rn->info; ri; ri = ri->next) {
      ...
      if (ri->attr && (CHECK_FLAG (ri->attr->flag,
  ...
  for (i = 0; i < BGP_STATS_MAX; i++) {
    ...
    switch (i) {
      ...
      case BGP_STATS_TOTPLEN:
        ...
        vty_out(vty, "%12.2f",
            ts.counts[i] ?
                (float) ts.counts[i] / (float) ts.counts[BGP_STATS_PREFIXES] :
                0);
        break;
...
static int bgp_table_stats_vty(struct vty *vty, const char *name,
  ...
  if (strncmp(afi_str, "ipv", 3) == 0) {
    ...
    if (strncmp(safi_str, "m", 1) == 0)
      ...
      vty_out(vty, "%% Invalid subsequent address family %s%s", safi_str,
          VTY_NEWLINE);
      return CMD_WARNING;
    }
  }
...
ALIAS(show_bgp_statistics, show_bgp_statistics_vpnv4_cmd,
...
ALIAS(show_bgp_statistics_view, show_bgp_statistics_view_vpnv4_cmd,
...
static const char *pcount_strs[] = {[PCOUNT_ADJ_IN] = "Adj-in", [PCOUNT_DAMPED
    ] = "Damped", [PCOUNT_REMOVED] = "Removed", [PCOUNT_HISTORY] = "History",
    [PCOUNT_STALE] = "Stale", [PCOUNT_VALID] = "Valid",
    [PCOUNT_ALL] = "All RIB", [PCOUNT_COUNTED] = "PfxCt counted", [PCOUNT_PFCNT
        ] = "Useable", [PCOUNT_MAX] = NULL,
  };
  ...
  for (rn = bgp_table_top(pc->table); rn; rn = bgp_route_next(rn)) {
    ...
    for (ri = rn->info; ri; ri = ri->next) {
      ...
      if (CHECK_FLAG (ri->flags, BGP_INFO_COUNTED)) {
        ...
        if (CHECK_FLAG (ri->flags, BGP_INFO_UNUSEABLE))
          plog_warn(peer->log, "%s [pcount] %s/%d is counted but flags 0x%x",
              peer->host,
        ...
        if (!CHECK_FLAG (ri->flags, BGP_INFO_UNUSEABLE))
          plog_warn(peer->log, "%s [pcount] %s/%d not counted but flags 0x%x",
              peer->host,
              inet_ntop(rn->p.family, &rn->p.u.prefix, buf, SU_ADDRSTRLEN),
              rn->p.prefixlen, ri->flags);
      }
...
static int bgp_peer_counts(struct vty *vty, struct peer *peer, afi_t afi,
  ...
  vty_out(vty, "%sCounts from RIB table walk:%s%s", VTY_NEWLINE, VTY_NEWLINE,
      VTY_NEWLINE);

  for (i = 0; i < PCOUNT_MAX; i++)
    vty_out(vty, "%20s: %-10d%s", pcount_strs[i], pcounts.count[i],
        VTY_NEWLINE);
...
static void show_adj_route(struct vty *vty, struct peer *peer, afi_t afi,
  ...
  if (!in && CHECK_FLAG (peer->af_sflags[afi][safi],
    ...
    vty_out(vty, "Originating default network 0.0.0.0%s%s", VTY_NEWLINE,
        VTY_NEWLINE);
    header1 = 0;
  }
...
/* old command */DEFUN (ipv6_mbgp_neighbor_received_routes,
...
ALIAS(show_ip_bgp_view_rsclient, show_ip_bgp_rsclient_cmd,
...
ALIAS(show_bgp_view_ipv4_safi_rsclient, show_bgp_ipv4_safi_rsclient_cmd,
...
DEFUN (show_ip_bgp_view_rsclient_route,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][SAFI_UNICAST],
      (argc == 3) ? argv[2] : argv[1], AFI_IP, SAFI_UNICAST, NULL, 0);
}
...
DEFUN (show_bgp_view_ipv4_safi_rsclient_route,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][safi],
      (argc == 4) ? argv[3] : argv[2], AFI_IP, safi, NULL, 0);
}
...
DEFUN (show_ip_bgp_view_rsclient_prefix,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][SAFI_UNICAST],
      (argc == 3) ? argv[2] : argv[1], AFI_IP, SAFI_UNICAST, NULL, 1);
}

ALIAS(show_ip_bgp_view_rsclient_prefix, show_ip_bgp_rsclient_prefix_cmd,
...
DEFUN (show_bgp_view_ipv4_safi_rsclient_prefix,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][safi],
      (argc == 4) ? argv[3] : argv[2], AFI_IP, safi, NULL, 1);
}
...
ALIAS(show_bgp_view_neighbor_routes, show_bgp_view_ipv6_neighbor_routes_cmd,
...
ALIAS(show_bgp_view_neighbor_damp, show_bgp_view_ipv6_neighbor_damp_cmd,
...
ALIAS(show_bgp_view_neighbor_flap, show_bgp_view_ipv6_neighbor_flap_cmd,
    "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) flap-statistics",
    SHOW_STR BGP_STR "BGP view\n" "BGP view name\n" "Address family\n" "Detailed information on TCP and BGP neighbor connections\n" "Neighbor to display information about\n" "Neighbor to display information about\n" "Display flap statistics of the routes learned from neighbor\n")

ALIAS(show_bgp_view_neighbor_routes, show_bgp_neighbor_routes_cmd,
    "show bgp neighbors (A.B.C.D|X:X::X:X) routes",
    SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n" "Neighbor to display information about\n" "Neighbor to display information about\n" "Display routes learned from neighbor\n")

ALIAS(show_bgp_view_neighbor_routes, show_bgp_ipv6_neighbor_routes_cmd,
    "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) routes",
    SHOW_STR BGP_STR "Address family\n" "Detailed information on TCP and BGP neighbor connections\n" "Neighbor to display information about\n" "Neighbor to display information about\n" "Display routes learned from neighbor\n")

/* old command */
ALIAS(show_bgp_view_neighbor_routes, ipv6_bgp_neighbor_routes_cmd,
...
ALIAS(show_bgp_view_neighbor_flap, show_bgp_neighbor_flap_cmd,
    "show bgp neighbors (A.B.C.D|X:X::X:X) flap-statistics",
    SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n" "Neighbor to display information about\n" "Neighbor to display information about\n" "Display flap statistics of the routes learned from neighbor\n")

ALIAS(show_bgp_view_neighbor_flap, show_bgp_ipv6_neighbor_flap_cmd,
    "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) flap-statistics",
    SHOW_STR BGP_STR "Address family\n" "Detailed information on TCP and BGP neighbor connections\n" "Neighbor to display information about\n" "Neighbor to display information about\n" "Display flap statistics of the routes learned from neighbor\n")

ALIAS(show_bgp_view_neighbor_damp, show_bgp_neighbor_damp_cmd,
    "show bgp neighbors (A.B.C.D|X:X::X:X) dampened-routes",
    SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n" "Neighbor to display information about\n" "Neighbor to display information about\n" "Display the dampened routes received from neighbor\n")

ALIAS(show_bgp_view_neighbor_damp, show_bgp_ipv6_neighbor_damp_cmd,
...
ALIAS(show_bgp_view_rsclient, show_bgp_rsclient_cmd,
...
ALIAS(show_bgp_view_ipv6_safi_rsclient, show_bgp_ipv6_safi_rsclient_cmd,
...
DEFUN (show_bgp_view_rsclient_route,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][SAFI_UNICAST],
      (argc == 3) ? argv[2] : argv[1], AFI_IP6, SAFI_UNICAST, NULL, 0);
}

ALIAS(show_bgp_view_rsclient_route, show_bgp_rsclient_route_cmd,
...
DEFUN (show_bgp_view_ipv6_safi_rsclient_route,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][safi],
      (argc == 4) ? argv[3] : argv[2], AFI_IP6, safi, NULL, 0);
}
...
DEFUN (show_bgp_view_rsclient_prefix,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][SAFI_UNICAST],
      (argc == 3) ? argv[2] : argv[1], AFI_IP6, SAFI_UNICAST, NULL, 1);
}

ALIAS(show_bgp_view_rsclient_prefix, show_bgp_rsclient_prefix_cmd,
...
DEFUN (show_bgp_view_ipv6_safi_rsclient_prefix,
  ...
  return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][safi],
      (argc == 4) ? argv[3] : argv[2], AFI_IP6, safi, NULL, 1);
}
...
u_char bgp_distance_apply(struct prefix *p, struct bgp_info *rinfo,
    struct bgp *bgp) {
  struct bgp_node *rn;
...
ALIAS(bgp_damp_unset, bgp_damp_unset2_cmd,
...
DEFUN (clear_ip_bgp_dampening_address_mask,
  ...
  return bgp_clear_damp_route(vty, NULL, prefix_str, AFI_IP, SAFI_UNICAST, NULL,
      0);
}

static int bgp_config_write_network_vpnv4(struct vty *vty, struct bgp *bgp,
...
int bgp_config_write_distance(struct vty *vty, struct bgp *bgp) {
  ...
  if (bgp->distance_ebgp && bgp->distance_ibgp && bgp->distance_local
    ...
    vty_out(vty, " distance bgp %d %d %d%s", bgp->distance_ebgp,
        bgp->distance_ibgp, bgp->distance_local, VTY_NEWLINE);
  ...
  for (rn = bgp_table_top(bgp_distance_table); rn; rn = bgp_route_next(rn))
    if ((bdistance = rn->info) != NULL ) {
      vty_out(vty, " distance %d %s/%d %s%s", bdistance->distance,
          ...
          bdistance->access_list ? bdistance->access_list : "", VTY_NEWLINE);
    }
